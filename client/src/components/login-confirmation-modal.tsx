...
export function LoginConfirmationModal({ isVisible, onClose, userName }: LoginConfirmationModalProps) {
  ...
  useEffect(() => {
    if (isVisible) {
      // Generate a new random greeting each time the modal becomes visible
      const newGreeting = getLoginGreeting();
      setGreeting(newGreeting);
      console.log('Login confirmation modal showing with message:', newGreeting);
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-start justify-center pt-8">
      ...
      {/* Modal */}
      <Card className="relative mx-4 w-full max-w-md animate-in slide-in-from-top-4 fade-in-0 duration-500 shadow-2xl animate-out fade-out-0 duration-1000" style={{ animationDelay: '6s' }}>
